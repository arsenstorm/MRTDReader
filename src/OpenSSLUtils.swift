...
public class OpenSSLUtils {
        ...
        X509_STORE_set_verify_cb(cert_ctx) { (ok, ctx) -> Int32 in
            ...
            if ok == 0 {
                ...
                if cert_error == X509_V_ERR_EC_KEY_EXPLICIT_PARAMS {
                    ...
                    Logger.openSSL.errorIfEnabled("Certificate verification error \(cert_error) at depth \(X509_STORE_CTX_get_error_depth(ctx))")
                }
            }
    ...
    static func verifyAndReturnSODEncapsulatedDataUsingCMS( sod : SOD ) throws -> Data {
        ...
        Logger.openSSL.debugIfEnabled("Verification successful");
    ...
    static func ASN1Parse( data: Data ) throws -> String {
        ...
        let _ = try data.withUnsafeBytes { (ptr) in
            ...
            if rc == 0 {
                ...
                Logger.openSSL.debugIfEnabled("Failed to parse ASN1 Data")
    ...
    static func generateAESCMAC( key: [UInt8], message : [UInt8] ) -> [UInt8] {
        ...
        Logger.openSSL.debugIfEnabled("aesMac - computed MAC (\(maclen) bytes)")
    ...
    public static func computeSharedSecret( privateKeyPair: OpaquePointer, publicKey: OpaquePointer ) -> [UInt8] {
        ...
        if keyType == EVP_PKEY_DH || keyType == EVP_PKEY_DHX {
            ...
            Logger.openSSL.debugIfEnabled("OpenSSLUtils.computeSharedSecret - DH secret len - \(len)")
        } else {
            ...
            if EVP_PKEY_derive_init(ctx) != 1 {
                // error
                Logger.openSSL.errorIfEnabled("EVP_PKEY_derive_init failed")
            }
            
            // Set the public key
            if EVP_PKEY_derive_set_peer( ctx, publicKey ) != 1 {
                // error
                Logger.openSSL.errorIfEnabled("EVP_PKEY_derive_set_peer failed")
            }
            ...
            if EVP_PKEY_derive(ctx, nil, &keyLen) != 1 {
                // Error
                Logger.openSSL.errorIfEnabled("EVP_PKEY_derive (get length) failed")
            }
            ...
            if EVP_PKEY_derive(ctx, &secret, &keyLen) != 1 {
                // Error
                Logger.openSSL.errorIfEnabled("EVP_PKEY_derive failed")
            }
            
            Logger.openSSL.debugIfEnabled("OpenSSLUtils.computeSharedSecret - secret len - \(keyLen)")
